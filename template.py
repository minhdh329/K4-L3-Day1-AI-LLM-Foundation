...
def call_openai(
    ...
    """Gọi OpenAI Chat Completions API và trả về (nội dung, độ trễ)."""
    import os
    import time
    ...
    actual_model = os.getenv("LAB_MODEL") if model == OPENAI_MODEL else model

    client = OpenAI(api_key=api_key, base_url=base_url)

    # Khai báo thời gian bắt đầu
...
def call_openai_mini(
    ...
    """Gọi OpenAI với mô hình mini (dùng LAB_MINI_MODEL)."""
    import os
    mini_model = os.getenv("LAB_MINI_MODEL", OPENAI_MINI_MODEL)
    return call_openai(
        prompt=prompt,
        model=mini_model,
        temperature=temperature,
...
# ---------------------------------------------------------------------------
def streaming_chatbot(system_prompt: str = None, model: str = OPENAI_MODEL):
    """
    ...
    while True:
        ...
        history = system_msgs + chat_msgs[-6:]
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                raise  # Hết lượt thử -> ném nguyên exception gốc ra ngoài
...
def run_assistant(
    ...
    while True:
        ...
        num_turns += 1
        total_tokens += count_tokens(user_msg, model=actual_model) + count_tokens(reply, model=actual_model)
        cost_info = estimate_cost(user_msg, reply, model=actual_model)
